...
@Configuration
public class LeituraDeFlatFileDelimitedReaderConfiguration {

    @Bean
    @StepScope
    public FlatFileItemReader<Seguro> leituraDeFlatFileDelimitedReader(
            @Value("#{jobParameters['arquivoSegurosDelimited']}") Resource resource) {
        return new FlatFileItemReaderBuilder<Seguro>()
                .name("leituraDeFlatFileDelimitedReader")
                .resource(resource)
                .delimited()
                .names("id", "nome", "tipo")
